...
# ── Model class (copied from EEGclassify.py) ──────────────────
...
# ── PREDICT ────────────────────────────────────────────────────
    ...
    if model is None:
        st.warning("⚠️ No trained model checkpoint found in `checkpoint/` folder. Please finish training first!")
        st.info("Training saves checkpoints every 100 steps. Once training finishes come back here!")
    else:
    ...
    if uploaded_file and model is not None:
        try:
            ...
            st.info(f"**Actual phrase:** {actual_text}　　**Actual category:** {actual_category}")
